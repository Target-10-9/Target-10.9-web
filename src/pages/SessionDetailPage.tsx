...
import { getPointsBySessionId } from '../features/points/pointsAPI';
...
const MiniTarget: React.FC<{ points: Point[]; size?: number }> = ({ points, size = 150 }) => {
    const rings: Ring[] = ringsData;

    return (
        <div style={{ width: size, height: size, borderRadius: 8, overflow: 'hidden', position: 'relative', border: '1px solid #ccc' }}>
...
export const SessionDetailPage: React.FC = () => {
    ...
    const [deleteWeaponModal, setDeleteWeaponModal] = useState<{ isOpen: boolean; weaponId: string | null; weaponName: string }>({
        isOpen: false,
        weaponId: null,
        weaponName: ''
    });
    ...
    // ---------- Fetch session / modes / weapons ----------
    useEffect(() => {
        if (!id) return;
        Promise.all([getSessionById(id), getSessionModes(), getWeapons()])
    ...
    // ---------- Fetch points et calcul score ----------
    useEffect(() => {
        if (!session?.id) return;

        const computeScore = (x: number, y: number) => {
            const dx = x;
            const dy = y;
            const dist = Math.sqrt(dx*dx + dy*dy);
            const normalizedDist = dist / 0.5;
            const ring = ringsData.find(r => normalizedDist >= r.radiusInner && normalizedDist < r.radiusOuter);
            return ring?.score || 0;
        };
        ...
        const fetchPoints = async () => {
            try {
                const sessionPoints = await getPointsBySessionId(session.id);
                const calculatedPoints = sessionPoints.map((p: any) => ({
                    ...p,
                    score: computeScore(p.x_Coordinate, p.y_Coordinate)
                }));
                setPoints(calculatedPoints);
                setTotalScore(calculatedPoints.reduce((sum: number, p: Point) => sum + p.score, 0));
            } catch (err) {
                console.error(err);
            }
        };
    ...
    const handleDeleteSessionClick = () => setDeleteSessionModal(true);
    ...
    const handleDeleteWeaponClick = (weaponId: string, weaponName: string) => {
        setDeleteWeaponModal({ isOpen: true, weaponId, weaponName });
    };
    ...
    const handleStartClick = () => setStartSessionModal(true);
    ...
    return (
        <>
            ...
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.6 }} className="relative">
                <motion.img layoutId={`session-image-${session.id}`} src={image} alt={session.name} className="w-full h-64 object-cover" />
                <div className="absolute inset-0 bg-gradient-to-b from-black/40 to-white/90" />
                <div className="absolute bottom-4 left-6 text-white">
                    {isEditing ? (
                        <>
                            <input type="text" name="name" value={formData.name || ''} onChange={handleInputChange} className="text-3xl font-bold text-black p-2 rounded flex-shrink-0" style={{ minWidth: '300px' }} />
                            <div className="flex items-center space-x-2">
                                <input type="datetime-local" name="dateStart" value={formData.dateStart || ''} onChange={handleInputChange} className="border p-2 rounded w-auto text-black" />
                                <span className="text-white">→</span>
                                <input type="datetime-local" name="dateEnd" value={formData.dateEnd || ''} onChange={handleInputChange} className="border p-2 rounded w-auto text-black" />
                            </div>
                        </>
                    ) : (
                        <>
                            <h1 className="text-3xl font-bold">{session.name}</h1>
                            <p className="text-sm italic">Du {new Date(session.dateStart).toLocaleDateString()} au {new Date(session.dateEnd).toLocaleDateString()}</p>
                        </>
                    )}
                    ...
                    <div className="mt-4 flex flex-wrap gap-3">
                        {session.etat !== 'Finished' ? (
                            !isEditing ? (
                                <>
                                    <button onClick={handleEditToggle} className="bg-[#123189] hover:bg-[#58628a] transition-colors text-white px-4 py-2 rounded-md shadow">Modifier</button>
                                    <button onClick={handleStartClick} className="bg-green-600 hover:bg-green-700 transition-colors text-white px-4 py-2 rounded-md shadow">Commencer la session</button>
                                </>
                            ) : (
                                <>
                                    <button onClick={handleSave} disabled={isSaving} className="bg-green-600 disabled:opacity-60 hover:bg-green-700 transition-colors text-white px-4 py-2 rounded-md shadow">{isSaving ? 'Sauvegarde...' : 'Sauvegarder'}</button>
                                    <button onClick={handleCancelEdit} className="bg-gray-300 hover:bg-gray-350 transition-colors text-gray-800 px-4 py-2 rounded-md shadow">Annuler</button>
                                </>
                            )
                        ) : null}

                        <button onClick={handleDeleteSessionClick} className="bg-red-600 hover:bg-red-700 transition-colors text-white px-4 py-2 rounded-md shadow">Supprimer</button>
                    </div>
                </div>
            </motion.div>

            {/* Content */}
            <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ duration: 0.6, delay: 0.2 }} className="px-6 py-8 space-y-8">
                ...
                <section className="flex flex-col md:flex-row md:gap-12">
                    <div className="flex-1">
                        ...
                        {isEditing ? (
                            <div className="space-y-2">
                                ...
                                <select name="sessionModeId" value={formData.sessionModeId || ''} onChange={handleInputChange} className="border p-2 rounded w-full md:w-1/2">
                                    <option value="">-- Sélectionner un mode --</option>
                                    {sessionModes.map(mode => <option key={mode.id} value={mode.id}>{mode.name} - {mode.discipline}</option>)}
                                </select>
                            </div>
                        ) : (
                            <>
                                {session.sessionModes ? (
                                    <>
                                        ...
                                        <p className="text-sm text-gray-500">Temps limite : {session.sessionModes.timeLimits} | Échauffement : {session.sessionModes.warmUp}</p>
                    ...
                    <aside className="w-full md:w-64 bg-gray-50 p-4 rounded-md shadow-md flex flex-col justify-center items-center mt-6 md:mt-0">
                        <h3 className="text-lg font-semibold mb-2 text-gray-700">Points réalisés</h3>
                        {points.length > 0 ? (
                            <p className="text-2xl font-bold text-indigo-600">{totalScore} pts</p>
                        ) : (
                            <p className="text-gray-500 italic text-center">
                                {new Date(session.dateStart) > new Date() ? 'Session pas commencée' : 'Aucun point enregistré'}
                            </p>
                        )}
                    </aside>
                    ...
                    {points.length > 0 && (
                        <div className="flex justify-center items-center mt-6 md:mt-0">
                            <MiniTarget points={points} size={200} />
                        </div>
                    )}
                    ...
                    <div className="flex items-center mb-2">
                        ...
                        {isEditing && (
                            <div className="flex gap-2 items-center ml-4">
                                <select value={newWeapon.weaponId} onChange={(e) => setNewWeapon({ weaponId: e.target.value })} className="border p-2 rounded">
                                    <option value="">-- Choisissez une arme --</option>
                                    {weapons.map((weapon) => (
                                        <option key={weapon.id} value={weapon.id}>{weapon.name}</option>
                                    ))}
                                </select>
                                <button onClick={handleAddWeapon} disabled={isAddingWeapon || !newWeapon.weaponId} className="bg-blue-500 disabled:opacity-60 hover:bg-blue-600 text-white px-4 py-2 rounded">{isAddingWeapon ? 'Ajout...' : 'Ajouter'}</button>
                            </div>
                        )}
                    </div>

                    {session.sessionModes?.sessionModeWeaponDetails?.length > 0 ? (
                            ...
                            {session.sessionModes.sessionModeWeaponDetails.map((detail: any) => (
                                <li key={detail.id} className="bg-gray-100 p-4 rounded-md shadow-sm flex justify-between items-center">
                                    <div>
                                        ...
                                        {detail.weaponDetails.description && <p className="text-sm italic text-gray-600">{detail.weaponDetails.description}</p>}
                                        {detail.weaponDetails.serialNumber && <p className="text-sm italic text-gray-600">N° de série : {detail.weaponDetails.serialNumber}</p>}
                                    </div>
                                    {isEditing && (
                                        <button onClick={() => handleDeleteWeaponClick(detail.weaponDetails.id, detail.weaponDetails.name)} className="text-red-500 hover:text-red-700 ml-4" title="Supprimer arme autorisée" disabled={isDeletingWeaponId === detail.weaponDetails.id}>
            ...
            <ConfirmModal
                ...
                inputProps={{
                    ...
                    value: targetId,
                    onChange: (val) => { setTargetId(val); setStartError(null); }
                }}
                errorMessage={startError ?? undefined}
                onConfirm={async (inputValue) => {
                    if (!inputValue || inputValue.trim() === '') { setStartError("Vous devez renseigner l'ID de la cible !"); return; }
                    if (!id || !session) return;
                    try {
                        const sessionModeId = session.sessionModeId ?? session.sessionModes?.id;
                        if (!sessionModeId) return;
                        const payload = { etat: "InProgress", sessionModeId, name: session.name, dateStart: session.dateStart, dateEnd: session.dateEnd, targetId: inputValue.trim() };
                        await updateSession(id, payload);
                        const updated = await getSessionById(id);
                        setSession(updated);
