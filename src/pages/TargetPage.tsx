...
function useManualCountdown(initialSeconds: number) {
    ...
    const reset = (newSec?: number) => {
        const val = newSec ?? initialSeconds;
        setSeconds(val);
        setInitial(val);
        setRunning(false);
    };
...
const TimerControl: React.FC<{ label: string; timer: ReturnType<typeof useManualCountdown> }> = ({ label, timer }) => (
    <div style={{ display: 'flex', alignItems: 'center', marginTop: 12, gap: 12 }}>
        <div style={{ width: 60, height: 60 }}>
            <CircularProgressbar
                value={timer.seconds}
                maxValue={Math.max(timer.initial, 1)}
...
const TargetRings: React.FC<{ onHit: (position: THREE.Vector3) => void }> = ({ onHit }) => (
    <group>
        {rings.map((r, i) => (
            <mesh
                key={i}
                onPointerDown={(e) => {
                    e.stopPropagation();
                    const pos = e.point;
                    onHit(new THREE.Vector3(pos.x, pos.y, 0));
                }}
                position={[0, 0, i * 0.0005]}
            >
                            ...
                            <div style={{ minWidth: 340, maxHeight: 800, overflow: 'auto', background: '#fff', padding: 20, borderRadius: 12, boxShadow: '0 4px 12px rgba(0,0,0,0.15)' }}>
                                ...
                                <div style={{ marginTop: 16 }}>
                                    ...
                                    <div style={{ maxHeight: 260, overflow: 'auto', padding: '8px 10px', border: '1px solid #e5e7eb', borderRadius: 8, background: '#fafafa', fontFamily: 'ui-monospace, SFMono-Regular, Menlo, Monaco, Consolas, "Liberation Mono", "Courier New", monospace', fontSize: 14 }}>
                                        {shotsDetailed.length === 0 ? <div style={{ color: '#6b7280' }}>Aucun tir pour le moment.</div> :
                                            shotsDetailed.map((line, idx) => <div key={idx} style={{ padding: '2px 0' }}>{line}</div>)
                                        }
                                    </div>
                                    <div style={{ marginTop: 20 }}><h3 style={{ fontSize: 16, fontWeight: 500 }}>Tirs effectués : {shotsFired}</h3></div>
                                </div>
                            </div>
                        </div>
